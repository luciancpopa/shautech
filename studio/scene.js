...
function chair(parent,x,z){box(.7,.12,.7,mats.dark,parent,x,.48,z);box(.7,.85,.1,mats.dark,parent,x,.94,z+.3);box(.07,.45,.07,mats.trim,parent,x,.25,z);for(const a of [-1,1])box(.65,.05,.06,mats.trim,parent,x,.08,z+a*.2)}
function desk(parent,x,z,color){box(3,.13,1.4,mats.wood,parent,x,.84,z);for(const a of [-1,1])box(.12,.8,1.1,mats.dark,parent,x+a*1.3,.4,z);box(1.35,.83,.07,mats.dark,parent,x,1.42,z-.25);label('shautech',chapters[color].name.toUpperCase(),1.22,.68,parent,x,1.42,z-.201,95,chapters[color].color);box(.08,.32,.08,mats.trim,parent,x,1,z-.25);box(.48,.03,.3,mats.trim,parent,x,.92,z-.25);box(.6,.035,.24,mats.dark,parent,x,.925,z+.27);chair(parent,x,z-1.2)}
...
for(let i=1;i<8;i++){
 ...
 const title=label(ch.name.toUpperCase(),ch.eyebrow,5.3,2.3,group,0,2.7,-8.35,105,ch.color);title.userData.action={kind:'details',index:i};hits.push(title);
 const info=label('EXPLORE','CLICK FOR THE STORY',1.25,.62,group,0,1.1,-2.9,100,ch.color);info.userData.action={kind:'details',index:i};hits.push(info);box(1.35,1.4,.5,mats.dark,group,0,.65,-3.1);
 desk(group,0,-6,i);chair(group,-1.4,-4.5);chair(group,1.4,-4.5);
 const light=new T.PointLight(col,25,14);light.position.set(0,3.8,-4);group.add(light);
 const exhibit=new T.Group();exhibit.position.set(-2.9,1.75,-4.8);group.add(exhibit);box(1.55,1.1,1.55,mats.dark,group,-2.9,.55,-4.8);
 const emat=new T.MeshStandardMaterial({color:col,emissive:col,emissiveIntensity:.22,metalness:.55,roughness:.25});
 if(i===1){for(let a=-1;a<=1;a++)for(let b=-1;b<=1;b++)box(.28,.28,.28,emat,exhibit,a*.4,b*.4,0);}
 if(i===2){const globe=new T.Mesh(new T.IcosahedronGeometry(.75,2),new T.MeshBasicMaterial({color:col,wireframe:true}));exhibit.add(globe);const r=ring(.84,.016,col,exhibit,0,0,0,Math.PI/2);tick.push(t=>r.position.y=Math.sin(t)*.6)}
 if(i===3){for(let k=0;k<4;k++){const g=box(.5,.12,.45,emat,exhibit,Math.sin(k*1.5)*.45,(k-1.5)*.4,Math.cos(k*1.5)*.45);}}
 if(i===4){for(let k=0;k<5;k++){const m=new T.Mesh(new T.OctahedronGeometry(.25),emat);m.position.set(Math.sin(k/5*Math.PI*2)*.7,Math.cos(k/5*Math.PI*2)*.7,0);exhibit.add(m)}}
 if(i===5){figure('assets/lucian-studio.png',1.4,2.15,group,-1.45,0,-6.4,{kind:'details',index:i});figure('assets/andrei-studio.png',1.4,2.1,group,1.5,0,-6.4,{kind:'details',index:i});}
 if(i===6){for(let k=0;k<6;k++)box(1.1,.08,.8,emat,exhibit,0,(k-2.5)*.18,0)}
 if(i===7)ring(.75,.1,col,exhibit,0,0,0);
 tick.push(t=>exhibit.rotation.y=t*.2);
 // Tall cabinets make each office a place, not a floating label.
 for(let k=0;k<3;k++){box(.8,2.7,.65,mats.dark,group,3.6,1.35,-4-k*1.1);for(let j=0;j<5;j++)box(.66,.015,.66,mats.trim,group,3.6,.45+j*.46,-4-k*1.1);box(.02,2.1,.03,emissive(col,.8),group,3.18,1.4,-3.66-k*1.1)}
...
function beginRoom(i){if(!doors[i]||transition)return;roomIndex=i;roomReady=false;const d=doors[i],start=camera.position.clone();const end=new T.Vector3(d.side*6.1,1.7,d.z-(d.side?0:2.1));const curve=new T.CatmullRomCurve3(d.side?[start,new T.Vector3(d.side*1.6,1.7,d.z+2.8),new T.Vector3(d.side*4.3,1.7,d.z),end]:[start,new T.Vector3(0,1.7,d.z+3),new T.Vector3(0,1.7,d.z+.25),end]);transition={kind:'enter',p:0,curve,fromLook:look.clone(),toLook:new T.Vector3(d.side*12.3,1.8,d.z-(d.side?0:8.3))};}
...
else{const d=doors[roomIndex];targetLook.set(d.side?d.side*12.3:mx*3,1.8-my*.8,d.side?d.z-mx*3:d.z-8.3);look.lerp(targetLook,1-Math.exp(-dt*5));}
camera.lookAt(look);
